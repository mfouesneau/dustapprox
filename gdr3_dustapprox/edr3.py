...
with A0 the extinction at
550 nm, as a function of the star's intrinsic color or effective temperature
(both denoted by X):
...
Riello et al. (2020) fit the above formula on a grid of extinctions convolving
the Gaia eDR3 passbands with Kurucz spectra (Castelli & Kurucz, 2003) and the
Fitzpatrick et al. (2019) extinction curve for solar metallicity, :math:`3500 K < T_{eff}
< 10000 K` in steps of :math:`250 K`, and :math:`0.01 < A_0 < 20 mag` with a step linearly
increasing with :math:`0.01 mag`.

Internally, they did a fit for main-sequence stars (:math:`log g > 4.5`) and for the
upper HR diagram (giants, upper MS, etc.) up to :math:`M_G \sim 5 mag`.
