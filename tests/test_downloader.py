...
class TestPrettySizePrint:
    ...
    def test_pretty_size_kilobytes(self):
        ...
        result = _pretty_size_print(
            2048
        )  # Use 2KB to avoid scientific notation
        assert "KB" in result
    ...
    def test_pretty_size_zettabytes(self):
        ...
        result = _pretty_size_print(
            2 * 1024 * 1024 * 1024 * 1024 * 1024 * 1024 * 1024
        )
        assert "ZB" in result
    ...
    def test_pretty_size_yottabytes(self):
        ...
        result = _pretty_size_print(
            2 * 1024 * 1024 * 1024 * 1024 * 1024 * 1024 * 1024 * 1024
        )
        assert "YB" in result
...
class TestDlAsciiProgress:
    ...
    def test_progress_length_customization(self):
        ...
        with patch("sys.stdout.write"), patch("sys.stdout.flush"):
            result = list(
                _dl_ascii_progress(data, total=40, progress_length=30)
            )
    ...
    def test_progress_mininterval(self):
        ...
        with patch("sys.stdout.write"), patch("sys.stdout.flush"):
            result = list(
                _dl_ascii_progress(data, total=100, mininterval=0.01)
            )
...
class TestDownloadFile:
    ...
    def test_download_file_no_content_length(self):
        ...
        mock_response.iter_content = Mock(
            return_value=[b"full content"]
        )  # Provide iter_content too
    ...
    def test_download_file_already_exists_size_match(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            mock_response = Mock()
            mock_response.headers = {
                "content-length": str(len(existing_content))
            }

            with patch("requests.get", return_value=mock_response):
                download_file(
                    "http://example.com/file", file_path, overwrite=False
                )
    ...
    def test_download_file_overwrite_true(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with patch("requests.get", return_value=mock_response):
                download_file(
                    "http://example.com/file", file_path, overwrite=True
                )
